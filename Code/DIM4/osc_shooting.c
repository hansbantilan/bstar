...
void derivs(double x, double y[], double dydx[])
{
	...
	dydx[k]= f4[j];
      ...
      /*dAdx = 1./3.*x* ( A*C*Dphidt^2 + A*dphidx^2 + A^2*phi^2  +  0   */	 
      f1[0]=   1./3.*x* (    uno[0]    +   dos[0]   +  tres[0]    +  0.5*GAMA*tres_prim[0])+
      /*    A/x - A^2/x    */ 
	 2.*a[0]/x - 2.*a2[0]/x ;
      /* This is the rr compenent of Einstein equation */
      /*                  dC/dx= (2.0/x)*( C  - A*C  */
      for(k=0;k<=nnn;k++) f2[k]= (4.0/x)*(c[k]-ac[k])+ 
      /*		    x*A*phi^2*C	 + 0 		*/
			    2./3.*x*cinco[k]   + 0.5*GAMA*x*cinco_prim[k];
      /* This is the Klein Gordon equation              */
      for(k=1;k<=nnn-1;k++){
	f3[k]= xi[k];
      /*d^2phidx^2 =  C * d^2phi/dt^2 +  0.5*C*phi   + A*phi     + 0    */	
	f4[k]      =  seis[k]         +  0.5*cgot[k] + siete[k]  + GAMA*siete_prim[k]
      /*  - dphidx/x + 1./3.*x*A*dphidx*phi^2  +             0            - 2*A*dphidx/x */
	  - xi[k]/x  + 1./3.*x*ocho[k]         +  0.25*GAMA*x*ocho_prim[k]- 2*axi[k]/x;
      }
